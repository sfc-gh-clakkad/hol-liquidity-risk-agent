...
DROP DATABASE IF EXISTS IDENTIFIER('LIQUIDITY_RISK_DB');
...
REVOKE CREATE SNOWFLAKE INTELLIGENCE ON ACCOUNT FROM ROLE IDENTIFIER($role_name);
...
--   'DROP DATABASE IF EXISTS LIQUIDITY_RISK_DB;' || CHR(10) ||
--   'DROP WAREHOUSE IF EXISTS IDENTIFIER(''LIQUIDITY_RISK_WH_'' || $user_name);' || CHR(10) ||
...
--   'REVOKE CREATE SNOWFLAKE INTELLIGENCE ON ACCOUNT FROM ROLE IDENTIFIER($role_name);' || CHR(10) ||
